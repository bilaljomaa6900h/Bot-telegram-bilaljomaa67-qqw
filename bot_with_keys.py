...
# Gemini API URL - عنوان API لجيميني
GEMINI_API_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-pro:generateContent"

# In-memory storage for user data - تخزين بيانات المستخدمين في الذاكرة
users = {}
last_update_id = 0
...
# Text translations for bot messages - ترجمات نصوص رسائل البوت
TRANSLATIONS = {
    "welcome_message": {
...
def get_text(key, language="en"):
    ...
    if key not in TRANSLATIONS:
        ...
        # الرجوع إلى رسالة افتراضية إذا لم يتم العثور على المفتاح
    ...
    if language in TRANSLATIONS[key]:
        ...
        # الرجوع إلى الإنجليزية إذا لم تكن اللغة المطلوبة متاحة
        return TRANSLATIONS[key]["en"]

def get_user_data(users, user_id):
    """
    Get user data from the in-memory storage.
    الحصول على بيانات المستخدم من التخزين في الذاكرة.
    """
    if user_id not in users:
        # Initialize with default values if user not found
        # تهيئة بقيم افتراضية إذا لم يتم العثور على المستخدم
        users[user_id] = {
            "name": "Unknown",
            "username": None,
            "language": "en",
...
def add_message_to_context(users, user_id, role, content):
    ...
    إضافة رسالة إلى سياق محادثة المستخدم.
    """
    user_data = get_user_data(users, user_id)
    ...
    # تقييد السياق بآخر 10 رسائل لتجنب حدود الرموز
...
def get_user_count(users):
    ...
    الحصول على إجمالي عدد المستخدمين الفريدين.
    """
    return len(users)
...
def send_typing_action(chat_id):
    ...
    إرسال إجراء الكتابة للإشارة إلى أن البوت يقوم بإنشاء رد.
    """
    payload = {
        "chat_id": chat_id,
        "action": "typing"
...
def generate_gemini_response(prompt):
    ...
    إنشاء رد باستخدام واجهة برمجة تطبيقات Gemini.
    """
    headers = {
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            # الرجوع إلى ردود أساسية في حالة فشل API
        ...
        # إذا وصلنا إلى هنا، فقد حدث خطأ ما في تحليل الرد
...
def process_update(update):
    ...
    معالجة تحديث واحد من تيليجرام.
    """
    global users
    
    # Skip updates without messages
    # تخطي التحديثات بدون رسائل
    ...
    if text.startswith("/"):
        ...
        if command == "/start":
            ...
            # أمر غير معروف، التعامل معه كرسالة عادية
...
def handle_normal_message(user_id, chat_id, text):
    ...
    # تعيين موجه النظام استنادًا إلى لغة المستخدم
...
def main():
    ...
    # Starting bot (all required API keys are already set)
    # بدء تشغيل البوت (جميع مفاتيح API المطلوبة تم تعيينها بالفعل)
    ...
    while True:
        try:
            ...
            # الحصول على التحديثات من تيليجرام
            params = {
...
2. ببساطة قم بتشغيل البوت باستخدام: python bot_with_keys.py
